/**
 * Project catalogue — v6.8.1 OrbitDesk + v3.1 Chokepoint — Updated for world-class portfolio
 */
...
export const projects: Project[] = [
  {
    ...
    tech: [
      ...
      "v6.8.0: Advanced NLP engine rewrite — calculateCommunicationScore from dumb keyword to Gong/Chorus/Grammarly: empathy (apology+acknowledgment+reassurance+personalization), clarity (structure+simple vs technical+readability 8-20w/sent), technical (logs+tool+RCA+remediation), fluency (filler um/uh/repetition/confidence), clientLanguage (Bloom 😅 vs NovaTech Correlation ID vs Apex SEC-2024-07). QA weighted tech40+clar25+emp15+flu10+lang10. CommunicationChannel intelligent replies context-aware not random. v6.8.1: Phone out-of-control fix — level-based critical-only: Lvl1 no auto calls focus tickets, Lvl2 rare P1 1 per 4-8min, Lvl3 occasional 3-6min, Lvl4 moderate 2-5min, Lvl5+ realistic 1.5-4min max6/hr, critical pool filter P1||breach||timeLeft<5min. Overlap fix: phone idle moved top-68 right-4 not bottom-4 overlapping Messaging dock, Missed Calls top-120, incoming toast right-360 lg:right-720 left of chat windows (360/680), active call z70 above Messaging z65. Build 9/9 routes, 0 vulns, Vercel gamma + PWA + Electron 32",
    ],
    concepts: [
